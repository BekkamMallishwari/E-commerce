(() => {
    ...
    async function loadProductReviews(productId) {
        ...
        try {
            const response = await AppUtils.apiRequest(`/products/${activeProductId}/reviews`);
    ...
    async function submitReview(event) {
        ...
        if (!AppUtils.requireLogin("Please sign in to review this product")) {
            return;
        }
        ...
        if (comment.length < 3 || comment.length > 1000) {
            AppUtils.notify("Review comment must be between 3 and 1000 characters", "error");
            return;
        }
